...
Zookeeper.prototype.getTopics = function (cb) {
  var topicsPath = '/brokers/topics';
  var self = this;
  logger.debug('get topics');
  async.waterfall(
    [
      function (callback) {
        self.client.exists(topicsPath, callback);
      },
      function (stat, callback) {
        if (stat) {
       
          self.client.getChildren(topicsPath, function (error, children, stats) {
            if (error) {
              logger.error(error);
              return callback(error);
            }
